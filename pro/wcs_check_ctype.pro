...
;     the projection type extracted from them.
; EXPLANATION:
;
;     Stops with an error message if CTYPE does not conform to standard,
;     unless one or both CTYPE strings is missing in which case it sets
;     coord_type = 'X' and projection_type = 'DEF'
;     
...
IF N_elements( ctype ) GE 1 THEN BEGIN
    ...
    IF bad_coord THEN BEGIN
        ...
        MESSAGE, 'Should be ''RA--'' or ''xLON'' or ''xxLN''', /continue
        RETURN
    ENDIF
    ...
    IF N_elements( ctype ) GE 2 THEN BEGIN
        ...
        if (projection_type ne STRUPCASE(STRMID(ctype2,5,3))) then begin
          ...
          message,'     5-8 of both CTYPE1 and CTYPE2.',/continue
          return
        endif
        ...
        IF bad_coord THEN BEGIN
            ...
            MESSAGE, 'Should be ''DEC-'' or ''xLAT'' or ''xxLT''', /CONTINUE
            RETURN
        ENDIF
